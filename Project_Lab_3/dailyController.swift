//This controller graphs the user data from their calorie intake
...
class dailyController: UIViewController, ChartViewDelegate{
    ...
    override func viewDidLayoutSubviews() {
        ...
        async{
            //getWeeklyCal makes a call to databse to retrieve users' entries over the week using the authorization token and userId as parametres
            let userPlot = try await getWeeklyCal(usID: UserCredentials!.3, token: UserCredentials!.1)
            //The calorie intake array from userPlot is set to a constant Oz_Array
            let kCal_Array = userPlot.0
            ...
            for x in 0..<7{
                ...
                //timePlot[x] + userPlot.2 uses the firstDay of the week returned from calling getWeeklyCal and generates the actual day of the month by adding a number from 0 to 6 to it and that is used for the x axis while the water intake data is used for the y axis.
                entries.append(BarChartDataEntry(x: Double(timePlot[x]+userPlot.2), y: kCal_Array[x]))
            }
            let set = BarChartDataSet(entries: entries)
            //The label of the BarChart is set here
            set.label = "User Calorie Consumption"
    ...
    func getWeeklyCal(usID:Int,token:String) async throws -> (Array<Double>,Array<Double>,Int){
        ...
        //the .startofWeek and .endOfWeek methods are called and formatted into "yyyy-MM-dd"
...
//The get month function returns the month of the year as a string given an integer representation from 1-12.
public func getMonth() ->(String){
    //figuring out the month of the year
    var month: String
    let date = Date()
    let formatter = DateFormatter()
    formatter.dateFormat = "MM"
    let date_Result = formatter.string(from: date)
    let someCharacter: String = date_Result
...
//This extension of the Date() object allows the startOfWeek and endOfWeek methods used in getWeeklyOz and getWeeklyCal.
extension Date {
...
//This is the form in which the result of the database request is formatted into.
struct WeeklyResult: Codable{
    let healthMetricId:Int
    let userId:Int
